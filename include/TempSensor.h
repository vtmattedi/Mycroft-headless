...
/// 1-Wire data pin for the DS18B20. The bus needs an external 4.7k pull-up to 3.3V -- the ESP32's
...
#else
/// Not 7 here: GPIO6-11 on the classic ESP32 are wired to the SPI flash, and driving one of them
/// takes the chip down. GPIO4 is free and is not a strapping pin.
...
/// Only bus I/O and the odd log line run on this task -- no JSON or MQTT -- so it needs less than
/// LIGHT_TASK_STACK.
#define TEMP_TASK_STACK 3072
/// Same slot as LIGHT_TASK_PRIORITY: above the Arduino loop task (1), far below WiFi (18+).
